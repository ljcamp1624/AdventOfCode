...
posList1 = GetPosList(path1);
posList2 = GetPosList(path2);

[idxList1, idxList2] = ismember(posList1(:, 1:2), posList2(:, 1:2), 'rows');
overlaps = posList1(idxList1, 1:2);
dists = sum(abs(overlaps), 2);
bestDist = min(dists(dists ~= 0));

travDists = sum([posList1(idxList1, 3), posList2(idxList2(idxList1), 3)], 2);
bestTravDist = min(travDists(travDists ~= 0));

function posList = GetPosList(path)
commaIdx = strfind(path, ',')';
moveIdx = [[1; commaIdx + 1], [commaIdx - 1; length(path)]];
posList = [0, 0, 0];
for i = 1:size(moveIdx, 1)
    dir = path(moveIdx(i, 1));
    val = str2double(path((moveIdx(i, 1) + 1):(moveIdx(i, 2))));
    currPos = posList(end, 1:2);
    currDist = posList(end, 3);
    d = (1:val)';
    if strcmp(dir, 'R')
        currPos = currPos + [d, zeros(size(d))];
    elseif strcmp(dir, 'L')
        currPos = currPos + [-d, zeros(size(d))];
    elseif strcmp(dir, 'D')
        currPos = currPos + [zeros(size(d)), -d];
    elseif strcmp(dir, 'U')
        currPos = currPos + [zeros(size(d)), d];
    end
    posList = [posList; [currPos, currDist + d]];
end
end